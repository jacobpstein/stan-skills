#!/usr/bin/env bash
#
# Install stan-skills for AI coding assistants
#
# Usage:
...
#   ./install.sh gemini -- stan-modeling      # Specific skill to Gemini CLI
...
validate_skills() {
    ...
    if [[ -f "$hooks_file" ]]; then
        ...
        local scripts
        scripts=$(jq -r '.. | .command? // empty' "$hooks_file" 2>/dev/null | sed -n 's/.*\${CLAUDE_PLUGIN_ROOT}\///p' || true)
